...
class DetectingProcess():
    ...
    def MultiOtsu(self,hist, num_classes):
        if num_classes<2:
            ...
            return thresholds
        if num_classes>4:
            original_distri = list(hist[0])
            original_bins = list(hist[1])
            #downsample
            distri = np.array(original_distri[::5])
            bins = np.array(original_bins[::5])
            hist_new = (distri, bins)
...
def RegionDivid(mask):
    ...
    for i in range(len(rps)):
        corordinates = rps[i].coords.T
        temp_mask = np.zeros_like(mask)
        temp_mask[tuple(corordinates)] = 1.0
        masks.append(temp_mask)
    return masks